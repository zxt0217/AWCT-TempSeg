#!/usr/bin/env python3
"""Final polished SCI refinement for the AWCT-TempSeg framework figure.
...
def flat_skip(slide, x0, x1, y_top, y_base):
    color = "D6D6D6"
    arrow(slide, x0, y_base, x0, y_top, color, 0.50, head=False)
    arrow(slide, x0, y_top, x1, y_top, color, 0.50, head=False)
    arrow(slide, x1, y_top, x1, y_base, color, 0.50)
...
def build_compact(prs):
    ...
    arrow(slide, 10.50, 1.57, 10.50, 2.62, "CFE6C9", 0.55, dash=True, head=False)
    arrow(slide, 10.50, 2.62, 4.20, 2.62, "CFE6C9", 0.55, dash=True, head=False)
    arrow(slide, 4.20, 2.62, 4.20, vfb_y, "CFE6C9", 0.55, dash=True)
...
def parse_args():
    ...
    p.add_argument("--output", default=str(Path.home() / "Desktop" / "AWCT_TempSeg_Fig2_final_polished.pptx"))
    return p.parse_args()
...
def main():
    ...
    prs.save(out)
    print(f"Wrote final polished editable PPTX to {out}")
