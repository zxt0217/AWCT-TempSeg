#!/usr/bin/env python3
"""Final ready-for-submission refinement for the AWCT-TempSeg framework figure.
...
def tiny_legend(slide):
    x, y = 10.38, 2.48
    text(slide, x, y, 0.39, 0.11, [("Legend", 4.8, True, C.ink)])
    items = [
    ...
    for i, (code, desc, fill) in enumerate(items):
        yy = y + 0.140 * (i + 1)
        box(slide, x + 0.08, yy, 0.24, 0.10, [(code, 3.9, True, C.ink)], fill, C.gray_line, 0.38)
        text(slide, x + 0.36, yy - 0.004, 1.30, 0.10, [(desc, 3.95, False, C.dim)])
    arrow(slide, x + 0.08, y + 0.72, x + 0.41, y + 0.72, C.line, 0.55)
    text(slide, x + 0.49, y + 0.675, 0.86, 0.10, [("Feature stream", 3.95, False, C.dim)])
    arrow(slide, x + 0.08, y + 0.92, x + 0.41, y + 0.92, "DCB78F", 0.50, dash=True)
    text(slide, x + 0.49, y + 0.875, 1.04, 0.10, [("Curriculum feedback", 3.95, False, C.dim)])
...
def build_compact(prs):
    ...
    # Single light curriculum feedback loop: Next-stage sampling -> mini-batch sampler.
    arrow(slide, 9.02, 3.89, 9.02, 4.12, "D6A777", 0.60, dash=True, head=False)
    arrow(slide, 9.02, 4.12, 1.02, 4.12, "D6A777", 0.60, dash=True, head=False)
    arrow(slide, 1.02, 4.12, 1.02, 2.59, "D6A777", 0.60, dash=True, head=False)
    arrow(slide, 1.02, 2.59, 1.18, 2.59, "D6A777", 0.60, dash=True)
...
def parse_args():
    ...
    p.add_argument("--output", default=str(Path.home() / "Desktop" / "AWCT_TempSeg_Fig2_final_ready_submission.pptx"))
    return p.parse_args()
...
def main():
    ...
    prs.save(out)
    print(f"Wrote final ready submission editable PPTX to {out}")
