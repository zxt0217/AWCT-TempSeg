#!/usr/bin/env python3
"""Ready-for-paper light refinement for the AWCT-TempSeg framework figure.
...
def tiny_legend(slide):
    x, y = 7.86, 3.96
    text(slide, x, y, 0.38, 0.11, [("Legend", 4.8, True, C.ink)])
    items = [
    ...
    for i, (code, desc, fill) in enumerate(items):
        yy = y + 0.145 * (i + 1)
        box(slide, x + 0.42, yy, 0.24, 0.10, [(code, 3.9, True, C.ink)], fill, C.gray_line, 0.40)
        text(slide, x + 0.70, yy - 0.004, 1.40, 0.10, [(desc, 4.0, False, C.dim)])
    arrow(slide, x + 2.36, y + 0.18, x + 2.68, y + 0.18, C.line, 0.60)
    text(slide, x + 2.76, y + 0.130, 0.82, 0.10, [("Feature stream", 4.0, False, C.dim)])
    arrow(slide, x + 2.36, y + 0.41, x + 2.68, y + 0.41, "D9AE82", 0.50, dash=True)
    text(slide, x + 2.76, y + 0.360, 1.05, 0.10, [("Curriculum feedback", 4.0, False, C.dim)])


def flat_skip(slide, x0, x1, y_top, y_base):
    color = "E1E1E1"
    arrow(slide, x0, y_base, x0, y_top, color, 0.42, head=False)
    arrow(slide, x0, y_top, x1, y_top, color, 0.42, head=False)
    arrow(slide, x1, y_top, x1, y_base, color, 0.42)
...
def build_compact(prs):
    ...
    arrow(slide, 10.50, 1.57, 10.50, 2.62, "DDEED8", 0.42, dash=True, head=False)
    arrow(slide, 10.50, 2.62, 4.20, 2.62, "DDEED8", 0.42, dash=True, head=False)
    arrow(slide, 4.20, 2.62, 4.20, vfb_y, "DDEED8", 0.42, dash=True)
    ...
    arrow(slide, 9.02, 3.89, 9.02, 4.02, "D8A677", 0.50, dash=True, head=False)
    arrow(slide, 9.02, 4.02, 2.48, 4.02, "D8A677", 0.50, dash=True, head=False)
    arrow(slide, 2.48, 4.02, 2.48, 2.59, "D8A677", 0.50, dash=True)
...
def parse_args():
    ...
    p.add_argument("--output", default=str(Path.home() / "Desktop" / "AWCT_TempSeg_Fig2_ready_for_paper.pptx"))
    return p.parse_args()
...
def main():
    ...
    prs.save(out)
    print(f"Wrote ready-for-paper editable PPTX to {out}")
