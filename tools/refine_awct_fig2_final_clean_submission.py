#!/usr/bin/env python3
"""Final clean submission refinement for the AWCT-TempSeg framework figure.
...
def tiny_legend(slide):
    x, y = 10.62, 2.58
    text(slide, x, y, 0.36, 0.10, [("Legend", 4.5, True, C.ink)])
    items = [
    ...
    for i, (code, desc, fill) in enumerate(items):
        yy = y + 0.130 * (i + 1)
        box(slide, x + 0.08, yy, 0.22, 0.09, [(code, 3.7, True, C.ink)], fill, C.gray_line, 0.36)
        text(slide, x + 0.34, yy - 0.004, 1.22, 0.09, [(desc, 3.7, False, C.dim)])
    arrow(slide, x + 0.08, y + 0.67, x + 0.38, y + 0.67, C.line, 0.52)
    text(slide, x + 0.46, y + 0.625, 0.82, 0.09, [("Feature stream", 3.7, False, C.dim)])
    arrow(slide, x + 0.08, y + 0.86, x + 0.38, y + 0.86, "DCB78F", 0.45, dash=True)
    text(slide, x + 0.46, y + 0.815, 0.98, 0.09, [("Curriculum feedback", 3.7, False, C.dim)])


def flat_skip(slide, x0, x1, y_top, y_base):
    color = "E7E7E7"
    arrow(slide, x0, y_base, x0, y_top, color, 0.36, head=False)
    arrow(slide, x0, y_top, x1, y_top, color, 0.36, head=False)
    arrow(slide, x1, y_top, x1, y_base, color, 0.36)
...
def build_compact(prs):
    ...
    # Reduced, flat cross-layer links.
    flat_skip(slide, 2.78, 7.38, 0.74, 0.99)
    flat_skip(slide, 4.28, 8.10, 0.68, 0.99)
    ...
    box(slide, 1.18, 2.45, 1.30, 0.28, [("Mini-batch sampling", 5.6, True, C.ink), ("WeatherWeightedSampler", 4.3, False, C.dim)], C.purple_light, C.purple_dark, 0.9)
    arrow(slide, 1.82, 2.45, 1.98, 1.69, "D4A06B", 0.55, dash=True)
    ...
    # Pale orthogonal validation feedback line to the validation statistics.
    arrow(slide, 9.90, 1.57, 9.90, 2.52, "E2F0DE", 0.42, dash=True, head=False)
    arrow(slide, 9.90, 2.52, 4.20, 2.52, "E2F0DE", 0.42, dash=True, head=False)
    arrow(slide, 4.20, 2.52, 4.20, vfb_y, "E2F0DE", 0.42, dash=True)
    ...
    # Light curriculum feedback path: Next-stage sampling -> mini-batch sampler.
    arrow(slide, 9.02, 3.89, 9.02, 4.18, "DDB389", 0.42, dash=True, head=False)
    arrow(slide, 9.02, 4.18, 7.80, 4.18, "DDB389", 0.42, dash=True)
    arrow(slide, 3.15, 4.18, 1.02, 4.18, "DDB389", 0.42, dash=True, head=False)
    arrow(slide, 1.02, 4.18, 1.02, 2.59, "DDB389", 0.42, dash=True, head=False)
    arrow(slide, 1.02, 2.59, 1.18, 2.59, "DDB389", 0.42, dash=True)
...
def parse_args():
    ...
    p.add_argument("--output", default=str(Path.home() / "Desktop" / "AWCT_TempSeg_Fig2_final_clean_submission.pptx"))
    return p.parse_args()
...
def main():
    ...
    prs.save(out)
    print(f"Wrote final clean submission editable PPTX to {out}")
